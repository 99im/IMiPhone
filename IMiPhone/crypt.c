...
void keyRevert(char* originalKey,char* revertKey){
    char tempKey[17];
    tempKey[16] = '\0';
    originalKey[16] = '\0';
    revertKey[16] = '\0';
    int i;
    strcpy(tempKey,originalKey);
    for( i = 0; i < 16;i++ ){
		revertKey[i] = ((tempKey[i]  + tempKey[(i+1)%16] )*2 + i)%126 + 1;
	}
	strcpy(tempKey,revertKey);
	for( i = 0; i < 16;i++ ){
		revertKey[i] = ((tempKey[i]  + tempKey[(i+2)%16] )*3 + i)%126 + 1;
	}
	strcpy(tempKey,revertKey);
	for( i = 0; i < 16;i++ ){
		revertKey[i] = ((tempKey[i]  + tempKey[(i+3)%16] )*4 + i)%126 + 1;
	}
}
